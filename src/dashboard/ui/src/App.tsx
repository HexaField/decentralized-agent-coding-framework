  ...
  return (
    <div class="w-full h-screen flex flex-col">
      ...
      <header class="border-b p-3 flex items-center gap-2">
        ...
        <nav class="ml-auto flex gap-2">
          <For each={tabs}>
            {(t) => (
              <button
                class={`px-4 py-2 rounded border ${activeTab() === t ? 'bg-black text-white' : 'bg-white'}`}
        ...
        <button class="ml-2 px-3 py-2 border rounded" onClick={() => setIsChatOpen((v) => !v)}>
          {isChatOpen() ? '>' : '<'}
        </button>
      </header>
      ...
      >
            ...
            >
              ...
              <aside class="border rounded p-2 overflow-auto" style={{ 'grid-row': '1 / span 2' }}>
                ...
                <For each={agents()}>
                  {(a) => (
                    <button
                      class={`w-full text-left border rounded p-2 mb-2 ${selectedAgent() === a.name ? 'bg-slate-200' : ''}`}
              ...
              <Show when={!!selectedAgent()}>
                <section class="grid gap-3" style={{ 'grid-template-columns': '1fr 1fr' }}>
                  <div class="border rounded overflow-hidden flex flex-col">
                    ...
                    <pre class="flex-1 bg-slate-50 p-2 overflow-auto whitespace-pre-wrap">
                      {agentLogsText()}
                    </pre>
                  </div>
                  <div class="border rounded overflow-hidden flex flex-col">
                    ...
                    <pre class="bg-slate-50 p-2 h-28 overflow-auto whitespace-pre-wrap">
                      {agentPromptLog().join('\n')}
                    </pre>
                  </div>
                </section>
        ...
        <aside
          class="border-l p-3 flex flex-col"
          ...
          <Show when={isDev}>
            <div class="mt-3 text-xs">
              ...
              <div class="flex gap-2 mb-2">
                <button class="px-2 py-1 border rounded" onClick={manualEnsure}>
                  Ensure Agent
                </button>
                <button
              ...
              <pre class="bg-slate-100 p-2 h-32 overflow-auto whitespace-pre-wrap">
                {debugLines().join('\n')}
              </pre>
            </div>
          </Show>
