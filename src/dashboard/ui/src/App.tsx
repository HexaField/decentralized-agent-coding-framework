...
export default function App() {
  ...
  async function startTaskStream(id: string, pre: HTMLPreElement) {
    pre.textContent = ''
    try {
      const out = await fetch(`/api/taskLogs?id=${encodeURIComponent(id)}`).then((r) => r.json())
      pre.textContent = (out.lines || []).join('\n')
      pre.scrollTop = pre.scrollHeight
    } catch {}
    const es = new EventSource(`/api/stream/task?id=${encodeURIComponent(id)}`)
    es.onmessage = (e) => {
  ...
  async function startAgentStream(name: string, pre: HTMLPreElement) {
    pre.textContent = ''
    try {
      const out = await fetch(`/api/agentLogs?name=${encodeURIComponent(name)}`).then((r) => r.json())
      pre.textContent = (out.lines || []).join('\n')
      pre.scrollTop = pre.scrollHeight
    } catch {}
    const es = new EventSource(`/api/stream/agent?name=${encodeURIComponent(name)}`)
    es.onmessage = (e) => {
      pre.textContent += (pre.textContent ? '\n' : '') + e.data
      pre.scrollTop = pre.scrollHeight
    }
                  ...
                  <div class="p-2 space-y-2">
                    <div class="flex gap-2 items-center">
                      <input class="border p-2" placeholder="local port (e.g., 8450)" onKeyDown={(e) => {
                        if(e.key==='Enter'){
                          const p = (e.currentTarget as HTMLInputElement).value.trim(); if(p) setEditorSrc(`/embed/local/${encodeURIComponent(p)}/`)
                        }
                      }} />
                      <button class="px-3 py-2 bg-indigo-600 text-white rounded" onClick={(e)=>{
                        const input = (e.currentTarget.previousSibling as HTMLInputElement)
                        const p = input.value.trim(); if(p) setEditorSrc(`/embed/local/${encodeURIComponent(p)}/`)
                      }}>Open Editor</button>
                    </div>
